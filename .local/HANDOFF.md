...
| **P2.1-2.4** | `refactor/sheet-struct` | `b6c38db` | Çok sayfa: `Sheet` struct + çalışma-kopyası modeli, TÜM sayfaları yükle, Ctrl+PgUp/PgDn gezinme, kenarlıkta `Beta (2/3)` göstergesi, sayfa başına imleç hafızası, CSV yüklemede workbook sıfırlama (yeni hata bulundu+düzeltildi) | **96/96** · clippy delta **0** · pty+pyte ile görsel doğrulama |
| **P2.5** | `feat/mouse-support` | `747aa6b` | Mouse: saf `hit_test` modülü, tık/sürükle/tekerlek/başlık/sayfa-göstergesi, fail-closed modal koruması | **119/119** · clippy delta **0** · SGR protokol testi geçti |
...
## SIRADAKİ: P3.1 — Okuma katmanını umya-spreadsheet'e geçir

**Branch:** `feat/xlsx-write`, `feat/mouse-support`'tan (çok sayfa + mouse üzerine).
Tasarım §3.1/§3.2 hazır, umya round-trip AMPİRİK kanıtlı. Sıra: P3.1 oku → P3.2 kirli takip → P3.3 yaz.

---

## AÇIK KONU: fiziksel ydotool tıklaması uygulamaya ulaşmadı

SGR protokol testi (gerçek binary, gerçek pty) mouse'un ÇALIŞTIĞINI kanıtladı: tık→hücre,
sürükle→`A2:C5`, satır/sütun başlığı, sayfa göstergesi→Beta. Ama `ydotool` ile ekranda yapılan
fiziksel tıklama Ghostty penceresindeki rustxl'e ulaşmadı (imleç A1'de kaldı).

Denenen: cihaz-piksel koordinatı (5760×2160) → başarısız; ekranın 2× ölçekli olduğu tespit edildi,
mantıksal koordinata (2880×1080) bölündü → yine başarısız. ydotoold çalışıyor (PID 1285),
`ydotool click 0xC0` hata vermiyor.

Kalan hipotezler (sıradaki oturumda test edilecek): (a) ydotool uinput mutlak eksen aralığı
ekran boyutuyla eşleşmiyor, (b) pencere odak/konum sorunu, (c) Ghostty mouse raporlama modu.
**rustxl kodu şüpheli DEĞİL** — protokol katmanı kanıtlandı.

---

## (TAMAMLANDI) P2.1 — `Sheet` struct'ını ayır
